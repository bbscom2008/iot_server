<template>
  <div class="container">
    ...
    <el-card class="mt-10">
      ...
      <my-table
        class="mt-10"
        :loadData="getProviderAccountChangePage"
        :tableData="tableData"
        ref="tableRef"
        style="width: 100%"
        border
...
<script>
import { getProviderAccountChangePage ,providerAccountChangeExport} from '@/api/cash-manage'
import { formatDateTime } from '@/utils'
import { cashRules, queryRules } from './rules'
...
export default {
  ...
  methods: {
    ...
    async onDialogBtnOk() {
      ...
      let res = await providerAccountChangeExport(rest)
      this.$message.success('导出成功')
      this.onDialogBtnCancel()
      console.log(res, 'res')
      FileSaver.saveAs(res, '码商账变记录.xlsx')
    },
    exportReport() {
      this.dialogVisible = true
      this.exportTime = ''
    },
    async getProviderAccountChangePage(page) {
      ...
      let res = await getProviderAccountChangePage(rest, { ...page })
