<template>
  <div class="container">
    <el-card>
      ...
      >
        ...
        <el-form-item label="业务类型" prop="businessType">
          <el-input value="业务类" disabled size="small" />
        </el-form-item>
    ...
    <el-card class="mt-10">
      ...
      <my-table
        class="mt-10"
        :loadData="getMerchantAgentAccountChangePage"
        :tableData="tableData"
        ref="tableRef"
        style="width: 100%"
        border
        ...
        <el-table-column label="账变类型" align="center">
          <template slot-scope="{ row }">
            ...
            <span v-if="row.changeType == '3'"
              >冲正
              <el-tooltip class="item" effect="dark" content="订单失败后，返还金额" placement="top">
...
<script>
import { getMerchantAgentAccountChangePage ,merchantAgentAccountChangeExport} from '@/api/cash-manage'
...
export default {
  ...
  methods: {
    ...
    async onDialogBtnOk() {
      ...
      let res = await merchantAgentAccountChangeExport(rest)
      this.$message.success('导出成功')
      this.onDialogBtnCancel()
      console.log(res, 'res')
      FileSaver.saveAs(res, '商户代理账变记录.xlsx')
    },
    exportReport() {
      this.dialogVisible = true
      this.exportTime = ''
    },
    handlePickerOneDay(e) {
      console.log('--handlePickerOneDay---', e)
    },
    async getMerchantAgentAccountChangePage(page) {
      let obj = { ...this.queryFormData, tenantId: this.tenantId, userType: '3' }
      ...
      let res = await getMerchantAgentAccountChangePage(rest, { ...page })
