<template>
  <div class="container">
    <el-card>
      <el-form :inline="true" :rules="queryRules" ref="queryFormData" :model="queryFormData" class="demo-form-inline">
...
:default-time="['00:00:00', '23:59:59']"
    ...
    <el-card class="mt-10">
      ...
      <my-table
        class="mt-10"
        :loadData="getMerchantWithdrawalPage"
        :tableData="tableData"
        ref="tableRef"
        style="width: 100%"
        border
      >
        ...
        <el-table-column label="操作" align="center" prop="action" fixed="right">
          <template slot-scope="{ row }">
            <el-button v-if="row.withdrawalApplyStatus == '0'" type="text" size="small" icon="el-icon-edit" @click="showShenHeDialog(row)">
              审核
            </el-button>
          </template>
        </el-table-column>
...
<script>
import { getMerchantWithdrawalPage, merchantWithdrawalAudit,exportMerchantWithdrawal } from '@/api/merchant-cash'
...
export default {
  ...
  methods: {
    async exportReport() {
      ...
      let res = await exportMerchantWithdrawal(obj)
      console.log(res, 'res')
      FileSaver.saveAs(res, '商户提现审核列表.xlsx')
    },
    async getMerchantWithdrawalPage(page) {
      ...
      let res = await getMerchantWithdrawalPage(rest, { ...page })
    ...
    showShenHeDialogBtnOk(e) {
      this.$refs.formData.validate(async (valid) => {
        if (valid) {
          ...
          let res = await merchantWithdrawalAudit({
            withdrawalApplyId,
            auditDesc,
            tradePassword,
