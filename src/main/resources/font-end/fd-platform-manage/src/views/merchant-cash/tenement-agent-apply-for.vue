...
<script>
import {
  getMerchantAgentWithdrawalPage,
  merchantAgentWithdrawalAudit,
  exportMerchantAgentWithdrawal
} from '@/api/merchant-cash'
...
export default {
  ...
  methods: {
    async exportReport() {
      ...
      let res = await exportMerchantAgentWithdrawal(obj)
      console.log(res, 'res')
      FileSaver.saveAs(res, '商户代理提现审核列表.xlsx')
    },
    showShenHeDialogBtnOk(e) {
      this.$refs.formData.validate(async (valid) => {
        if (valid) {
          ...
          let res = await merchantAgentWithdrawalAudit({
            withdrawalApplyId,
            auditDesc,
            tradePassword,
