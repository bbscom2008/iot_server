<template>
  <div class="container">
    ...
    <el-dialog :visible.sync="dialogVisible" title="提现申请" width="600px">
      ...
      >
        <el-form-item label="平台管理账号" prop="userName">
          <el-input
            v-model="formData.userName"
            placeholder="请输入平台管理账号"
            size="small"
            disabled
          />
        </el-form-item>
...
<script>
import { getTenantWithdrawalPage, tenantWithdrawalAdd,exportTenantWithdrawal } from '@/api/merchant-cash'
...
export default {
  ...
  methods: {
    async exportReport() {
      ...
      let res = await exportTenantWithdrawal(obj)
      console.log(res, 'res')
      FileSaver.saveAs(res, '平台提现申请列表.xlsx')
    },
    ...
    showAddDialog() {
      ...
      console.log(this.$store.state.userAccount, 'this.$store.state.user')
      this.formData = {
        userName: sessionStorage.getItem('loginForm.username'),
        can_use_money: this.$store.state.userAccount.availableBalance,
        withdrawalApplyAmount: '',
        payeeInfo: '',
        withdrawalApplyDesc: '',
        tradePwd: '',
    ...
    onDialogBtnOk() {
      this.$refs.formData.validate(async (valid) => {
        if (valid) {
          const { userName, can_use_money, ...obj } = this.formData
          let res = await tenantWithdrawalAdd(obj)
