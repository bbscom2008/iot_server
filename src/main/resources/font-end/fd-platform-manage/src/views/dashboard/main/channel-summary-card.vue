<template>
  <div class="card-box" v-loading="isLoading">
    ...
    <!-- <div class="chart-box" v-if="channelBarItem" >
      <ChannelBar
        width="100%"
        height="400px"
        class="item_channelName_class"
        id="item_channelName_id"
        :item="channelBarItem"
      ></ChannelBar>
    </div> -->
...
import { channelDailyReportGetReportPage } from '@/api/report-center'
...
export default {
  ...
  mounted() {
    ...
    getChannelByTenantId(this.tenantId).then((res) => {
  ...
  methods: {
    ...
    async channelDailyReportGetReportPage() {
      try {
        ...
        this.isLoading = true
        let res = await channelDailyReportGetReportPage(obj, this.page)
