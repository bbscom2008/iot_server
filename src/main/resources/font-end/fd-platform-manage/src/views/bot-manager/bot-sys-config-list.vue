<template>
  <div class="container">
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="{ row }">
            ...
            <el-tag style="cursor: pointer"
              type="primary"
              size="small" @click="showUpdateDialog(row)"
            >更新配置
            </el-tag
            >
          </template>
    ...
    <el-dialog :visible.sync="dialogVisible" title="修改配置" width="600px" @close="onBtnCancel">


      <div >
        <h2 style="color: red; font-weight:600; text-align: center;"  > 非技术人员，请勿修改 </h2>
      </div>

     <!--  <el-select v-model="configInfo.configValue" placeholder="请选择"
                 v-if="['all', 'bank', 'wxPay', 'aliPay'].includes(configInfo.configValue)">
        <el-option
      ...
      <el-input
        ...
        v-model="configInfo.configValue"
        size="small"
        placeholder="请输入"
      /> -->
      ...
      >
        ...
        <el-form-item label="key" prop="configKey">
          <el-input v-model="formData.configKey" disabled  placeholder="输入key" size="small" />
        </el-form-item>
          ...
          <div class="input-line" v-for="(item, index) in formData.inputMessage" :key="index">
            <el-row v-if="formData.isEnable">
              ...
              <el-col :span="8">
                ...
                >
                  <el-input v-model="item.inputKey" placeholder="请输入key值"  disabled   size="small">
                  </el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
      ...
      <div style="margin-top: 10px; text-align: center;">
        <el-button type="primary" @click="onBtnOK()" size="small"> 更改配置</el-button>
...
export default {
  ...
  methods: {
    ...
    async botRegisterAgain() {
      let res = await botRegisterAgain(this.$store.state.tenant.currTenantId)
