...
export default {
  ...
  methods: {
      ...
      this.$refs.formData.validate(async(valid) => {
        if (valid) {
          if (this.isEdit) {
            ...
            //新增
            let res = await botDispatchUserAdd({ tenantId: this.$store.state.tenant.currTenantId, ...this.formData })
            console.log(res, '添加')
          }
          this.btnCancel()
          this.$refs.tableRef.getTableData(1)
    ...
    submitPasswordHandler() {
      this.twoStepPasswordForm.tenantId = this.$store.state.tenant.currTenantId
