...
export default {
  ...
  data() {
    return {
      ...
      orderStateOptions: [
        // { label: '', value: undefined, type: 'info' },
  ...
  methods: {
    ...
    onTabChange() {
      ...
      console.log('-----this.requestInfoData.requestPath----', this.requestInfoData.requestPath)
      if (
    ...
    async getCollectOrderFee() {
      let ret = await getCollectOrderFee(this.orderItem.orderId)
      if (ret.data) {
  ...
  mounted() {
    ...
    // this.getCollectOrderFee(this.orderItem.orderId)

    console.log('---mounted-----', this.payType)
    let createTime = this.orderItem.createTime
    let timestr = createTime.split(' ')
    let time = timestr.join('T')
    if (this.payType == 'collect') {
      // 代收订单详情
      console.log('----- this.orderItem.orderId---', this.orderItem.orderId)

      getCollectOrderDetail(this.orderItem.orderId, { createTime: time }).then((res) => {
      ...
      getPaymentOrderDetail(this.orderItem.orderId, { createTime: time }).then((res) => {
