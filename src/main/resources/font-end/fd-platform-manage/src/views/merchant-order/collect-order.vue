  ...
  <div class="container" :class="isShowUi ? 'isShowUi' : 'noUi'">
    ...
    <el-card class="mt-10 my-card">
      ...
      >
          ...
          <template slot-scope="{ row }">
            <div style="display: flex; flex-direction: column; align-items: flex-start">
              <div style="display: flex; align-items: center">
                <el-tag>通道</el-tag>
                <span style="padding: 0 5px">{{ row.channelName }}</span>
              </div>
        ...
        >
          <el-table-column label="商户信息" align="center" width="150">
            <template slot-scope="{ row }">
              <div style="display: flex; flex-direction: column; align-items: flex-start">
                <div>
                  <el-tag>用户名</el-tag>
          ...
          <el-table-column label="码商信息" align="center" width="150">
            <template slot-scope="{ row }">
              <div style="display: flex; flex-direction: column; align-items: flex-start">
                <div>
                  <el-tag>用户名</el-tag>
        ...
        <el-table-column align="center" label="操作" width="200" fixed="right">
          <template slot-scope="{ row }">
            <div class="opt-box">
              <div type="primary" class="btn mt-5" @click="showBlackDialog(row)">
...
import { getProviderOptionList, getProviderTop } from '@/api/service-list.js'
...
export default {
  ...
  watch: {
    selectedItems(newV) {
      console.log(newV)
      this.$store.commit('settings/SET_collectOrderColumns', newV)
    }
  },
  methods: {
    ...
    queryTableData() {
      this.$refs.queryFormRef.validate((valid) => {
        ...
        // 获得今日统计
        getTodayCollectOrderDataAnalysis(this.$store.state.tenant.currTenantId).then((res) => {
    ...
    allNotify() {
      this.$confirm('确认要将全部未通知成功的订单重新通知吗？')
        .then((_) => {
          batchNotifyMerchants(this.$store.state.tenant.currTenantId).then((res) => {
  ...
  mounted() {
    ...
    getYesterdayCollectOrderDataAnalysis(this.$store.state.tenant.currTenantId).then((res) => {
