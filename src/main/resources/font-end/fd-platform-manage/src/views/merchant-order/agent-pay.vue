  ...
  <div class="container" :class="isShowUi ? 'isShowUi' : 'noUi'">
    ...
    <el-card class="mt-10 my-card">
      <div class="tool-box">
        <div class="auto-flush">
          ...
          <el-dropdown trigger="click" style="margin-left: 10px; cursor: pointer;" >
            <span class="el-dropdown-link" style="display: flex; height: 100%; align-items: center; font-size: 24px;" >
            ...
            <el-dropdown-menu slot="dropdown" style="padding: 10px; width: 200px">
              ...
              <el-checkbox-group v-model="selectedItems" @change="handleItemChange">
                <el-checkbox :key="item" v-for="item in allItems" style="display: block;"  :label="item" >{{item}}</el-checkbox>
              </el-checkbox-group>
            </el-dropdown-menu>
          </el-dropdown>
      ...
      <my-table
        ...
        border
      >
        <el-table-column label="序号" align="center" type="index" width="40"  v-if="selectedItems.includes('序号')" />

        <el-table-column label="订单号" align="center" width="220"  v-if="selectedItems.includes('订单号')">
        ...
        <el-table-column label="产品信息" align="center" width="140"  v-if="selectedItems.includes('产品信息')">
          <template slot-scope="{ row }">
        ...
        <el-table-column label="交易者信息" align="center" width="200"  v-if="selectedItems.includes('交易者信息')">
        ...
        <el-table-column label="订单金额(元)" align="center" width="120"  v-if="selectedItems.includes('订单金额')">
          <template slot-scope="{ row }">
            {{ row.orderAmount }}
          </template>
        </el-table-column>

        <el-table-column label="收款信息" align="center" width="140" prop="paymentData"  v-if="selectedItems.includes('收款信息')">
          <template slot-scope="{ row }">
        ...
        <el-table-column label="订单时间" align="center" width="220px"  v-if="selectedItems.includes('订单时间')">
          <template slot-scope="{ row }">
            <div style="text-align: left">
        ...
        <el-table-column label="状态" align="center" width="160"  v-if="selectedItems.includes('状态')">
          <template slot-scope="{ row }">
            <div style="text-align: left">
              <div>
                订单状态:
        ...
        <el-table-column label="归属地" align="center" prop="submit_info"  v-if="selectedItems.includes('归属地')">
          <template slot-scope="{ row }">
            <el-button type="text" @click="look_card_area(row)">查看</el-button>
          </template>
        </el-table-column>

        <el-table-column label="转账凭证" align="center" prop="submit_info"  v-if="selectedItems.includes('转账凭证')">
          <template slot-scope="{ row }">
            <el-button
        ...
        <el-table-column label="审核用户名" align="center" prop="auditResultPeople" width="100"  v-if="selectedItems.includes('审核用户名')">
    ...
    >
      ...
      >
        ...
        <el-form-item v-if="[1, 2].includes(commonDialogType)" label="凭证" 
...
export default {
  ...
  data() {
    return {
      selectedItems: [],
      allSelected: true,
      allItems: ['序号', '订单号', '产品信息', '交易者信息', '订单金额', '收款信息', '订单时间', '状态', '归属地', '转账凭证', '审核用户名' ],
  ...
  methods: {
    ...
    queryTableData() {
      this.$refs.queryFormRef.validate((valid) => {
        ...
        // 获得今日统计
        getTodayPaymentOrderDataAnalysis(this.$store.state.tenant.currTenantId).then((res) => {
    ...
    onCommonBtnOk() {
      this.$refs.commonFormRef.validate(async (valid) => {
        ...
        if (this.chuliFormData.commonDialogType == 1) {
          ...
          //审核驳回
          this.chuliFormData.checkReason = 6 // 先没有固定原因，只有其他原因
    ...
    allNotify() {
      this.$confirm('确认要将全部未通知成功的订单重新通知吗？').then((_) => {
        batchNotifyPaymentMerchants(this.$store.state.tenant.currTenantId).then((res) => {
  ...
  mounted() {
    ...
    getYesterdayPaymentOrderDataAnalysis(this.$store.state.tenant.currTenantId).then((res) => {
