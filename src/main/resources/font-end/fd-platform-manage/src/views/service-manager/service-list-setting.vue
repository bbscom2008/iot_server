    ...
    >
      ...
      >
        ...
        <el-form-item label="码商费率" prop="providerRate">
          ...
          <div class="help-tip">如3%，每完成1笔订单，码商可得%3的利润</div>
        </el-form-item>
        <!-- 0 -->
        <el-form-item label="单笔费用" prop="providerPerTransactionFee">
          ...
          <div class="help-tip">如2元，每完成一笔订单，码商可得2元的利润</div>
        </el-form-item>
        <!-- 1 -->
  ...
  methods: {
    ...
    showPatchEditDialog(isModAll) {
      ...
      this.formPatch = {
        ...
        providerPerTransactionFee: '', // 单笔费率
        isActive: null // 0 保持不变 1 开启 2 关闭
      }
    },
    onPatchBtnCancel() {
      // this.$nextTick(() => {
      // if(this.$refs.formPatch){
    ...
    onPatchBtnOk() {
      this.$refs.formPatch.validate(async (valid) => {
        ...
        if (!this.isModAll) {
          // 批量
          const idList = this.selectionRows.map((ele) => ele.id)
