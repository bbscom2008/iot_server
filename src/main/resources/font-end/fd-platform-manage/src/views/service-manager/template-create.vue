          ...
          <el-tab-pane label="基础信息配置" name="tab1">
            ...
            <div class="mt-20 base-config">
              ...
              >
                  ...
                  >
                    ...
                    >
                      <img
                        ...
                        alt=""
                      />
                     <div :style="{textAlign:item.icon?'left':'center', width:item.icon?'auto':'100%'}" >{{ item.name }}</div>
                    </el-option>
                  </el-select>
                </el-form-item>

                 <el-form-item label="上传logo：" prop="userLogoFile" 
                ...
                <el-form-item label="模板主题：" prop="themeColor">
              ...
              >
                ...
                <template v-if="buttonConfigInfo.buttons.length > 0">
                  <div :key="index1" v-for="(btnInfo, index1) in buttonConfigInfo.buttons">
                    ...
                    <div :key="index" v-for="(item, index) in btnInfo.buttonParamList">
                      <el-form-item
                        label="参数："
                      ...
                      <el-form-item label="">
                        <el-checkbox v-model="item.isShow" :true-label="1" :false-label="0" >是否展示</el-checkbox>
                      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      baseInfo: {
        ...
        isDefault: 0 // 超管创建的 模板为 默认模板  ， 其他用户创建的模板为 非默认模板
      },
  ...
  methods: {
    ...
    async saveTemplate() {
      ...
      try {
        ...
        // 提交数据
        if (!this.cashTemplateId || this.baseInfo.isDefault == 1) {
          // 创建
          console.log('------create-----');
          

          // 根据默认模板新建模板时，删除不需要的ID
          // if(this.baseInfo.isDefault == 1){ 
    ...
    async getTemplateDetail() {
      ...
      // 如果是默认模板，清空模板名称 , 此时是平台端，复制了 超管端的 模板
      if(this.baseInfo.isDefault == 1){
        this.baseInfo.isDefault = 0
        this.cashTemplateId=''
        this.baseInfo.templateName = ''
        this.baseInfo.tenantId = this.$store.state.tenant.currTenantId
      }
  ...
  async mounted() {
    ...
    if (this.cashTemplateId && this.baseInfo.isDefault == 0 ) {
