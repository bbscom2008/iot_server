<template>
  <div class="container" :class="isShowUi ? 'isShowUi' : 'noUi'">
    ...
    <el-card class="mt-10">
      <!-- <div v-if="!isShowUi">
        ...
        <el-button
          ...
          >批量修改</el-button
        >
        <el-button type="success" plain icon="el-icon-plus" @click="showPatchDialog(true)"
          >全部修改</el-button
        >
        ...
        <el-button
          ...
          >批量添加产品</el-button
        >
        <el-button type="success" plain icon="el-icon-plus" @click="showPatchAddProductDialog(true)"
          >全部添加产品</el-button
        >
      ...
      >
        ...
        <!-- <el-table-column label="解锁" align="center" min-width="90">
          <template slot-scope="{ row }">
            <el-button type="text" @click="unloadUser(row)" >解锁</el-button>
          </template>
        </el-table-column> -->
        ...
        <el-table-column label="开启代收" align="center" min-width="90">
          <template slot-scope="{ row }">
            <el-switch v-model="row.isCollectEnable" @change="onBussinessSwitchChange(row)" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="备注信息" align="center" prop="remark" min-width="90" />
        <el-table-column label="创建时间" align="center" prop="createTime" min-width="90" />
    ...
    >
      ...
      >
        <el-form-item label="账号【建议用中文】" prop="userName" :rules="isEdit ? {} : userNameRule">
          <el-input
    ...
    >
      ...
      >
        ...
        <el-form-item label="最大接单数（代收）" prop="" v-if="formPatchData.isMaxConcurrentOrdersCount" >
          <el-input
            type="number"
        ...
        <el-form-item label="最大接单数（代付）" prop="" v-if="formPatchData.isMaxConcurrentOrdersCount">
          <el-input
            type="number"
        ...
        <div class="help-tip2" style="padding-left:120px;margin-bottom:20px;" v-if="formPatchData.isMaxConcurrentOrdersCount">
          <span  >
            最大接单数：一个码商同时最大可以接多少单，当前订单完成或者失败后，才可以接下一个订单，当设置为 0 时，接单数量不受限制
          </span>
        </div>
    ...
    <el-dialog
      ...
      :title="isAddAll ? modifyProvideLevel === 0 ? '全部添加产品' : '全部一级码商添加产品' : '批量添加产品'"
      :modal="false"
...
import {
  ...
  // allProvideAddProduct,
  // patchProvideAddProduct
...
import { systemUserIsLock } from '@/api/merchant.js'
...
`,
      ...
      this.$refs.formRef.validate(async (valid) => {
        if (valid) {
          ...
          if (this.isEdit) {
            ...
            await addService({
              ...this.formData,
              // userNickName: this.formData.userName,
      ...
      this.$refs.formPatchDataRef.validate(async (valid) => {
        if (valid) {
          ...
          if (this.isModAll) {
            ...
            let providerIds = this.selectionRows.map((ele) => ele.providerId)
    ...
    onCopyBtnOk() {
      this.$refs.copyFormDataRef.validate(async (valid) => {
        if (valid) {
          // 校验成功
          // 发起请求
          let res = await copyService(this.copyFormData)
    ...
    showChangeFeeDialog(row, type) {
      ...
      this.feeChangeFormData.changeAmount = ''
      this.feeChangeFormData.tradePassword = ''
    ...
    async loadTableData(page) {
      ...
      let res = await getServiceList(page, params)
    ...
    async getServiceListByTenantId() {
      //新增---获取上级码商
      // 获取上级码商
      let res = await getServiceListByTenantId(this.$store.state.tenant.currTenantId)
      this.serviceList = res.data
      console.log(res)
    },
    async getProviderOptionListExcludeChildren(providerId) {
      //修改---获取上级码商（不包含子级）
      let res = await getProviderOptionListExcludeChildren(this.$store.state.tenant.currTenantId, {
