  ...
  <div class="container">
    ...
    <el-card class="mt-10">
      ...
      <el-button
        ...
        >批量添加通道</el-button
      >
      <el-button type="success" plain icon="el-icon-plus" @click="showPatchAddChannelDialog(true)"
        >全部添加通道</el-button
      >
      <div class="tips mt-10">
        ...
        <span class="mr-10"
          >预付净额总和: {{ summaryData.collectDeposit ? (summaryData.collectDeposit - summaryData.availableBalance).toFixed(2) : 0 }}
        </span>
      </div>
      <my-table
        class="mt-10"
      ...
      >
        ...
        <el-table-column label="商户分组" align="center" prop="merchantPacket">
          <template slot-scope="scope">
            {{ scope.row.merchantPacket === 0 ? '代收' : '代付' }}
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="{ row }">
            <div class="fee-box">
              <span class="val">{{ Number(row.availableBalance).toFixed(2) }}</span>
        ...
        <el-table-column
          ...
          width="100px"
          sortable
          :sort-method="(a,b)=>(a.collectDeposit-a.availableBalance)-(b.collectDeposit-b.availableBalance)"
        >
          <template slot-scope="{ row }">
              <span class="val">{{ Number(row.collectDeposit - row.availableBalance).toFixed(2) }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="指定码商数量" align="center" prop="appointProviderCount">
          <template slot-scope="{ row }">
...
import { getChannelByTenantId, systemUserIsLock } from '@/api/merchant.js'
...
说明：（主要针对账号安全设定）
  ...
  3、初次登录后，请及时设置交易密码
    ...
    async onSwitchChange(row) {
      ...
      try {
        let res = await updateMerchantIsActive(row.merchantId, row.isActive)
        console.log(res)
      } catch (e) {
        console.log(e)
      }
    ...
    copyBtnOk() {
      this.$refs.copyFormData.validate(async (valid) => {
        if (valid) {
          await copyMerchant(this.copyFormData)
