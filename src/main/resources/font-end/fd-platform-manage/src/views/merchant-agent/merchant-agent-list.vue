    ...
    >
      ...
      <div slot="footer" style="text-align: center">
        <el-button
          ...
          >重置</el-button
        >
        <el-button @click="closeResetPassword()" type="success"> 关闭 </el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    async exportReport() {
      ...
      console.log(res, 'res')
      FileSaver.saveAs(res, '商户代理列表.xlsx')
    },
    ...
    onCopyBtnOk() {
      this.$refs.formCopyRef.validate(async (valid) => {
        if (valid) {
          await copyMerchantAgent(this.copyFormData)
...
商户代理管理后台：  ${this.user_info.loginAddress ? this.user_info.loginAddress : ''}
      ...
      this.$refs.formRef.validate(async (valid) => {
        if (valid) {
          if (this.isEdit) {
            ...
            // 新增
            await createMerchantAgent(this.formData)
