<template>
  <div
    ...
    :style="{
      backgroundImage: pcBackgroundImgUrl ? `url(${pcBackgroundImgUrl})` : ''
    }"
  ...
  >
    ...
    >
      <div class="title-container">
        <h3 class="title">{{ tenantName }} 平台端 - 登录页面</h3>
      </div>
      ...
      <!-- <div style="display: flex; margin: 20px; align-items: center" v-if="!isShowUi">
        <div style="color: #fff; margin-right: 10px">平台ID:</div>
        <el-input
          style="width: 300px"
          v-model="tempTenantId"
          placeholder="测试用，平台ID"
          type="text"
        />
      </div>  -->
...
export default {
  ...
  data() {
    return {
      ...
      otherQuery: {},
      isSaveUsername: true,
      addUrl: 'b6801a1bd4fa499ba8e42fa58b339d10-plt.fykknn.xyz', // 地址栏的完整地址，方便测试用
      domain: '', // 当前的域名
  ...
  computed: {
    ...
    pcBackgroundImgUrl() {
      if (
        ...
        console.log(this.$store.state.tenant.tenantInfo.pcBackImage.fileUrl,'this.$store.state.tenant.tenantInfo.pcBackImage.fileUrl')
    ...
    handleLogin() {
      this.$refs.loginForm.validate(async (valid) => {
        if (valid) {
          ...
          login({ ...this.loginForm, tenantId: this.tempTenantId, userType: '2' }, getDeviceType())
            .then((ret) => {
              ...
              try {
                ...
                this.$store.state.user.tokenInfo = decoded
                this.$store.state.userAccount.userId=decoded.userId
