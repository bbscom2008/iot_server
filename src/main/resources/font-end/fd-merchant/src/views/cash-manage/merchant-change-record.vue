<template>
  <div class="container">
    ...
    <el-card class="mt-10">
      ...
      <my-table
        class="mt-10"
        :loadData="getMerchantAccountChangePage"
        :tableData="tableData"
        ref="tableRef"
        style="width: 100%"
        border
      >
        ...
        <el-table-column label="业务类型" align="center">
          <template slot-scope="{ row }">
            <el-tag v-if="row.businessType == '2'" type="success"> 业务类 </el-tag>
            <el-tag v-if="row.businessType == '1'" type="success"> 代付类 </el-tag>
            <el-tag v-if="row.businessType == '0'" type="primary"> 代收类 </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="账变类型" align="center">
          <template slot-scope="{ row }">
            ...
            <span v-if="row.changeType == '3'">冲正</span>
            ...
            <span v-if="row.changeType == '9'">下级分佣</span>
...
<script>
import { getMerchantAccountChangePage, merchantAccountChangeExport } from '@/api/report-center'
...
export default {
  ...
  methods: {
    ...
    async onDialogBtnOk() {
      ...
      let res = await merchantAccountChangeExport(rest)
      this.$message.success('导出成功')
      this.onDialogBtnCancel()
      console.log(res, 'res')
      FileSaver.saveAs(res, '商户账变记录.xlsx')
    ...
    async getMerchantAccountChangePage(page) {
      let obj = { ...this.queryFormData, tenantId: this.tenantId, userType: '4' }
      ...
      let res = await getMerchantAccountChangePage(rest, { ...page })
