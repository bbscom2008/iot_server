  ...
  >
    ...
    >
      <div class="title-container">
        <h3 class="title">{{ tenantName }} 商户端 - 登录页面</h3>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      otherQuery: {},
      isSaveUsername: true,
      addUrl: 'test0010bc6f19b8d-mch.fykknn.xyz', // 地址栏的完整地址，方便测试用
      domain: '', // 当前的域名
  ...
  computed: {
    ...
    pcBackgroundImgUrl() {
      if (this.$store.state.tenant.tenantInfo && this.$store.state.tenant.tenantInfo.pcBackImage
  ...
  mounted() {
    ...
    console.log(sessionStorage.getItem('autoLogin-merchant'));
    
    if(this.$route.query.autoLogin){
      const autoLoginData = JSON.parse(sessionStorage.getItem('autoLogin-merchant'))
      if(autoLoginData){
        ...
        this.$nextTick(()=>{
          sessionStorage.removeItem('autoLogin-merchant')
          this.handleLogin()
        })
      }
    }
    ...
    handleLogin() {
      this.$refs.loginForm.validate(async (valid) => {
        if (valid) {
          ...
          login({ ...this.loginForm, tenantId: this.tempTenantId, userType: '4' }, getDeviceType())
            .then((ret) => {
