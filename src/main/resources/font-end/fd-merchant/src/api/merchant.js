...
// 分页查询当前商户下相关联的通道数据
export function getChannelByMerchantRelPage(data, params) {
  return request({
    url: tempBase + '/master/MerchantChannelRelation/getChannelMerchantRelationPage',
    method: 'post',
    data,
    params
  }) 
...
export function deleteChannelProductRelationById(data) {
  return request({
    url: tempBase + '/master/channelProductRelation/deleteChannelProductRelationById',
    method: 'post',
    data
  })
}
...
// 通过租户ID获取代付通道信息
export function getPaymentChannelListByTenantId(tenantId,params) {
  return request({
    url: tempBase + `/master/channel/getPaymentChannelListByTenantId/${tenantId}`,
    method: 'get',
    params
  })
}
