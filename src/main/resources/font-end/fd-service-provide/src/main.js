...
// import './utils/error-handler' // global error handler
...
import scrollDirective from '@/directive/scroll.js'
...
// 添加全局方法处理安全中心点击
Vue.prototype.openSecurityCenter = function () {
  store.commit('user/SET_SECURITY_CENTER_FLAG', true)
}
...
import UploadOverridePlugin from '@/plugins/upload-override';

// 然后覆盖 Upload 组件
Vue.use(UploadOverridePlugin, {
  showPasteHint: true,     // 显示粘贴提示
  autoUploadPaste: true,  // 粘贴后不自动上传
  pasteAccept: 'image/*'   // 只接受图片
});
...
// console.log('process.env.VUE_APP_CURR_MODE', process.env.VUE_APP_CURR_MODE);
// console.log('isShowDomainInput', process.env.VUE_APP_CURR_MODE === 'test');
...
Vue.mixin({
  ...
  data() {
    return {
      isShowDomainInput: process.env.VUE_APP_CURR_MODE === 'test', //开发为 false 生产为 true  
      // 全局变量
      myRules: myRules,
      // tenantId:store.state.user.tenantId|| localStorage.getItem('tenantId'),
      // 只能选三天内，或者，三天前的
      isShowUi: true, //永远为true
      myPickerOptions: {
        shortcuts: [
          {
            text: '今天',
        ...
        shortcuts: [
          ...
          },
          {
            ...
            onClick(picker) {
              ...
              const start = new Date(today_start.getTime() - 3600 * 1000 * 24 * 29)
              const end = new Date(today_end.getTime() - 3600 * 1000 * 24 * 3)
