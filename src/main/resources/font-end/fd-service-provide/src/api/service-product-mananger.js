...
// 分页查询产品 -- 码商端
export function getProductListForProcide({ pageNum, pageSize }, data) {
  return request({
...
export function getProductListByUserId() {
...
export function getAllProductType() {
  return request({
    url: tempBase + `/master/productType/getProductTypeListByCurrentUserId`,
    method: 'get'
  }) 
}
// 码商端-导出产品列表信息
export function exportByProvider(data) {
  return request({
    url: tempBase + `/master/product/exportByProvider`,
    method: 'post',
    data,
    responseType: 'blob'
  })
