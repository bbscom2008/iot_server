<template>
  <div class="picker-box">
    <el-popover placement="top" width="300" trigger="click" :hide-on-click="true">
      <div class="msg-list">
        <div class="msg-item" v-for="(msg, i) in cardList" 
      ...
      <div class="item" slot="reference">
        <i class="el-icon-postcard" style="font-size: 30px"></i>
        <span>快捷卡片</span>
      </div>
    </el-popover>
  </div>
...
export default {
  ...
  computed: {
    cardList() {
      return this.$store.state.chat.quickCardList
    }
  },
  mounted(){
  ...
  methods: {
    ...
    async getShortMessage() {
      if(this.$store.state.chat.quickCardList.length == 0){
    ...
    async sendTextMessage(msg) {
      ...
      this.$emit('select-card', messageItem)
