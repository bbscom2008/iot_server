  ...
  <div class="container">
    ...
    <el-card class="mt-10">
      ...
      >
        ...
        <el-table-column label="金额限制类型" align="center" min-width="110">
          <template slot-scope="{ row }">
            ...
            <span v-if="row.acceptOrderConfig.limitAmountType == '2'">
              <el-tag style="padding: 0 5px" size="small">{{
                row.acceptOrderConfig.minAmount
              }}</el-tag>
              ~
              <el-tag style="padding: 0 5px" size="small">{{
                row.acceptOrderConfig.maxAmount
              }}</el-tag>
            </span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="授权状态" align="center" min-winth="110" v-if="!isSaDan"  >
          <template slot-scope="{ row }">
            <el-tag style="margin-right: 0" v-if="row.authStatus == 1" type="success"
              >已授权</el-tag
            >
            <el-tag style="margin-right: 0" v-else-if="row.authStatus == 0" type="danger"
              >未授权</el-tag
            >
            <el-tag style="margin-right: 0" v-else-if="row.authStatus == 2" type="danger"
              >未监控</el-tag
            >
            <el-tag style="margin-right: 0" v-else-if="row.authStatus == 3" type="success"
              >已监控</el-tag
            >

            <div class="mt-10 ta-c" v-if="row.authStatus == 1 && row.uid">
              <el-tag type="success" style="padding-right: 0px; padding-left: 0px"
                >支付宝UID</el-tag
              >
              <div>
                <span
        ...
        <div v-show="batchImportStep === 2" style="text-align: center; padding: 30px 0">
          ...
          >
            <div style="color: #606266; line-height: 2">
              ...
              <div style="color: red; font-weight: bold">• 请修改收款码名称，防止重复!</div>
              <div>• 最多支持500条数据</div>
            </div>
          </div>
...
import {
  ...
  getProductById,
  exportPayeeAccountListForProvider,
...
// import { getProductInfoListByProviderId } from '@/api/service-list-setting.js'
...
export default {
  ...
  data() {
    return {
      ...
      payeeAccountNameRules: [
        ...
        //     // if (isEmpty(this.$store.state.user.personalInfo.providerId)) {
        ...
        //     const ret = await payeeAccountInfoNameIsExist(value, this.$store.state.user.personalInfo.providerId)
        //     if (ret.data) {
  ...
  computed: {
    isSaDan(){
      ...
      // this.$store.state.tenant.tenantInfo.tenantId === platformInfo["sadan"].tenantId ||
      // return this.$store.state.tenant.tenantInfo.tenantId === platformInfo["gerun"].tenantId || 
      //        this.$store.state.tenant.tenantInfo.tenantId === platformInfo["xiaoliu"].tenantId 
      return false
    },
    ...
    canBatchImportNext() {
      switch (this.batchImportStep) {
        case 0:
          // 步骤0：必须选择码商和产品
          return this.$store.state.user.personalInfo.providerId && this.formData.productId
        case 1:
  ...
  methods: {
    ...
    async loadAllPayeeAccountInfo() {
      let res = await getPayeeAccountListByProviderId(
        this.$store.state.user.personalInfo.providerId
      )
    ...
    async exportReport() {
      ...
      let res = await exportPayeeAccountListForProvider(params)
    ...
    async getOptionLists() {
      // 根据租户ID 获得所有的产品信息
      const providerId = this.$store.state.user.personalInfo
        ? this.$store.state.user.personalInfo.providerId
        : ''
      if (!providerId) {
        await this.$store.dispatch('user/getProviderPersonalInfo')
      }
      getProductInfoListByProviderId(this.$store.state.user.personalInfo.providerId).then((res) => {
        console.log('getProductInfoListByProviderId---', res)
        this.productList = res.data
        this.providerProductList = res.data
      })

      getProviderOptionList(this.$store.state.user.tenantId).then((res) => {
        console.log(res)
      ...
      // getProductListByProviderId(item)
      //   .then((res) => {
      //     console.log(res)
    ...
    handleProviderChanged(item) {
      ...
      this.fetchProductInfoListByProviderId(item)
    },
    async fetchProductInfoListByProviderId(item) {
    ...
    handleQrConfigError(error, file, fileList) {
      console.log('----handleQrConfigError----', error,  file.name, fileList.map(ele=>ele.name));
    ...
    async handleQrConfigSuccess(response, file, fileList) {
      console.log('----handleQrConfigSuccess----', response, file.name, fileList.map(ele=>ele.name));
      const fileName = file.name
      // 之前失败的再次上传，成功了
      const qrConfig = this.formData.payeeAccountConfig.qrConfig
      ...
      if (response && response.data) {
        ...
        const ret = await getFileById(response.data, true)
        console.log(ret)
    ...
    handleQrConfigChanged() {
      ...
      this.$set(this.formData.payeeAccountConfig.qrConfig, 'failedFiles', [])
      this.formData.payeeAccountConfig.qrConfig.uploadedFiles = []
    ...
    async uploadImage(action) {
      ...
      console.log('--uploadImage--fileList----', this.formData.payeeAccountConfig.qrConfig.fileList);
    ...
    async showEditDialog(row) {
      ...
      this.formData = Object.assign({}, row)
      this.fetchProductInfoListByProviderId(row.providerId)
      try {
        this.loadingInfo = true
        const res = await getPayeeAccountInfoById(row.payeeAccountId)
    ...
    async handleCreateWithAcceptConfig() {
      ...
      try {
        let allTask = []
        let providerId = this.$store.state.user.personalInfo.providerId
        let productId = formData.productId
        let remark = formData.remark
    ...
    buildAcceptOrderParams(form, payeeAccountIds) {
      return {
        ...
        payeeAccountExpireDuration:
          form.payeeAccountExpireTimeType == 2 ? Number(form.payeeAccountExpireDuration || 0) : 0,
    ...
    onConfigOrderBtnOk() {
      ...
      this.$refs.configOrderRef.validate(async (valid) => {
        ...
        if (valid) {
          const params = this.buildAcceptOrderParams(this.formData, [this.formData.payeeAccountId])
    ...
    async loadTableData(page) {
      ...
      const res = await getPayeeAccountInfoListPageByProvider(page, params)
