  ...
  <div class="container">
    ...
    <el-card class="mt-10">
      ...
      <div class="tips mt-10">
        汇总信息：
        <span class="mr-10">订单数量: {{ summaryData.orderCount ?summaryData.orderCount :0}} </span>
        <span class="mr-10">订单金额: {{ summaryData.orderAmount  ?summaryData.orderAmount :0}} </span>
        <span class="mr-10">成功订单数: {{ summaryData.successfulOrderCount  ?summaryData.successfulOrderCount :0}} </span>
        <span class="mr-10">成功订单金额: {{ summaryData.successfulOrderAmount  ?summaryData.successfulOrderAmount :0}} </span>
        <span class="mr-10">成功率(%): {{ summaryData.successfulRate  ?summaryData.successfulRate :0}}% </span>
      </div>

      <my-table
        class="mt-10"
        ref="tableRef"
        :loadData="productDailyReportGetReportPage"
        :tableData="tableData"
        style="width: 100%"
        border
      >
...
<script>
import {
  productDailyReportGetReportPage,
  productDailyReportSummary,
...
export default {
 ...
 methods: {
    async productDailyReportGetReportPage(page) {
      let obj = {
      ...
      let res = await productDailyReportGetReportPage(obj, page)
    ...
    async exportReport() {
      ...
      let res = await productDailyReportExport(obj)
      console.log(res, 'res')
    ...
    async onDialogBtnOk() {
      ...
      let res = await productDailyReportGenerate({
