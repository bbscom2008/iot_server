<template>
  <div class="container">
    <!-- <h2>下级账变</h2> -->
    <el-card>
      <el-form :inline="true" :rules="queryRules" :model="queryFormData" ref="queryFormData" class="demo-form-inline">
        ...
        <el-form-item label="业务类型" prop="businessType">
          <el-select v-model="queryFormData.businessType" placeholder="请选择业务类型" size="small">
        ...
        <el-form-item label="账变类型" prop="changeType">
          <el-select v-model="queryFormData.changeType" placeholder="请选择账变类型" size="small">
            <el-option label="全部" value=""></el-option>
        ...
        <el-form-item label="账变金额" prop="changeAmount">
          <el-input v-number-input type="number" v-model="queryFormData.changeAmount" placeholder="请输入账变金额" size="small">
...
:default-time="['00:00:00', '23:59:59']"
    ...
    <el-card class="mt-10">
      ...
      <my-table
        class="mt-10"
        :loadData="getProviderSubordinateAccountChangePage"
        :tableData="tableData"
        ref="tableRef"
        style="width: 100%"
        border
      >
        ...
        <el-table-column label="账变类型" align="center">
          <template slot-scope="{ row }">
            ...
            <span v-if="row.changeType == '3'">冲正<el-tooltip
                class="item"
                effect="dark"
                content="订单失败后，返还金额"
                placement="top"
            ...
            <span v-if="row.changeType == '9'">下级分佣<el-tooltip
                class="item"
                effect="dark"
                content="下级交易产生的利润"
                placement="top"
...
<script>
import { getProviderSubordinateAccountChangePage ,providerSubordinateAccountChangeExport} from '@/api/report-center'
...
export default {
  ...
  methods: {
    ...
    async onDialogBtnOk() {
      ...
      const { create_time, exportTime, ...rest } = obj
      let res = await providerSubordinateAccountChangeExport(rest)
      this.$message.success('导出成功')
      this.onDialogBtnCancel()
      console.log(res, 'res')
      FileSaver.saveAs(res, '码商下级账变记录.xlsx')
    },
    exportReport() {
      this.dialogVisible = true
      this.exportTime = ''
    },
    async getProviderSubordinateAccountChangePage(page) {
      let obj = { ...this.queryFormData, tenantId: this.tenantId, userType: '5' }
      ...
      let res = await getProviderSubordinateAccountChangePage(rest, { ...page })
