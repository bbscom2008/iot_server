            ...
            <view class="summary-grid" v-if="summaryInfo">
              ...
              <view class="summary-item">
                ...
                <text class="value red-txt"
                  >￥{{ summaryInfo.orderAmount }}</text
                >
              </view>
              <view class="summary-item">
                <text class="label">成功订单数</text>
                <text class="value">{{
                  summaryInfo.successfulOrderCount
                }}</text>
              </view>
              <view class="summary-item">
        ...
        <view v-else>
          ...
          >
            <view class="card-header">
              <view class="order-id"
                >统计日期:
                <text class="txt" style="margin-left: 10rpx">{{
                  item.statisticsDate
                }}</text>
              </view>
              <view class="info"
                >成功率:
                <u-tag :text="`${item.successfulRate} %`" type="success">
                </u-tag
              ></view>
            </view>
    ...
    >
      <view class="query-popup">
        ...
        <view class="query-buttons">
          <u-button icon="reload" class="reset-btn" @click="resetQueryForm()"
            >重置</u-button
          >
          <u-button icon="search" class="search-btn" @click="btnSearchClick()"
            >搜索</u-button
          >
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    async queryTableData() {
      ...
      // this.refreshTriggered = true
      let res = await productDailyReportGetReportPage(params, this.page)
      console.log(res)
      this.tableData = res.data.list
      this.total = res.data.total

      // 汇总信息
      let ret = await productDailyReportSummary(params, this.page)
      console.log('---productDailyReportSummary---', ret)
