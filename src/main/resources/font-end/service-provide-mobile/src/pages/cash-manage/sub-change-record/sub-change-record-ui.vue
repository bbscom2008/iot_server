			...
			<view class="main-content" v-else>
				<view v-for="(item, index) in tableData" :key="index" class="order-card">
					<view class="card-header">
						<text class="order-id">交易编号号: {{ item.accountChangeRecordId}}</text>
					...
					<view class="card-body">
						...
						<view class="info-row">
							<text class="info-label">码商名称:</text>
		...
		<uni-popup ref="queryPopupRef" :show="showFilter" @maskClick="closeSearchModal()">
			<view class="query-popup">
				...
				<scroll-view class="modal-content" scroll-y>
					<uni-forms ref="queryFormDataRef" :model="queryFormData" :rules="queryRules" label-position="top"
						...
						<uni-forms-item label="交易编号" name="accountChangeRecordId">
							<uni-easyinput v-model="queryFormData.accountChangeRecordId" type="number"
								placeholder="请输入交易编号, 纯数字" @blur="
                ...
                " />
						...
						<uni-forms-item label="交易编号" name="accountChangeRecordId">
							<uni-easyinput v-model="queryFormData.accountChangeRecordId" type="number"
								placeholder="请输入交易编号, 纯数字"
								@blur=" $refs.queryFormDataRef.validateField('accountChangeRecordId') " />
						</uni-forms-item>
						...
						<uni-forms-item label="码商名称" name="userNickName">
							<uni-easyinput v-model="queryFormData.userNickName" placeholder="请输入账号"
						...
						<uni-forms-item label="账变金额" name="changeAmount">
							<uni-easyinput v-model.number="queryFormData.changeAmount" placeholder="请输入账变金额"
								type="number" @blur="$refs.queryFormDataRef.validateField('changeAmount')" />
						</uni-forms-item>
...
<script>
	...
	import {
		getProviderSubordinateAccountChangePage
      ...
      onSubNavClick(v){
        ...
        this.queryTableData()
			...
			async queryTableData() {
				...
				// this.refreshTriggered = true
				let res = await getProviderSubordinateAccountChangePage(params, this.page)
				console.log(res);
