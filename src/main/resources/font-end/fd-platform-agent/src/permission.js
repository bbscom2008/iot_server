...
const whiteList = ['/login', '/auth-redirect', '/bindGoogleCode', '/changePassword', '/template-preview'] // no redirect whitelist
...
router.beforeEach(async(to, from, next) => {
  ...
  if (hasToken && googleCode && isChangePwd()) {
    if (whiteList.indexOf(to.path) !== -1) {
      ...
      // 获得所有的平台列表
      // if(store.getters.tenantList.length === 0){
      //   console.log("-----beforeEach----to.path- getTenantList--" + to.path);
      //   await store.dispatch('tenant/getTenantList')
      // }
      // 获得平台信息
      await store.dispatch('userAccount/getUserAccount')
