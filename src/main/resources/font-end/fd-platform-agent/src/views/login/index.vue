<template>
  <div
    ...
    :style="{
      backgroundImage: `url(${require('@/assets/img/default-bg.png')})`
    }"
  ...
  >
    ...
    >
      <div class="title-container">
        <h3 class="title">{{ '平台代理 - 登录页面' }}</h3>
      </div>

      <div v-if="!isShowUi" style="display: flex; margin: 20px 0">
      ...
      <div v-if="!isShowUi" style="display:flex; margin:20px; align-items: center;">
        <div style="color:#fff;margin-right: 10px;">平台ID: </div><el-input v-model="tempTenantId" style="width:300px;" placeholder="测试用，平台ID" type="text" />
      </div>
    ...
    <BindGoogleCode
      ...
      :is-first-login-password="isFirstLoginPassword"
    />
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      otherQuery: {},
      isSaveUsername: true,
      addUrl: '48e14d77871462c39-tenantagent.fykknn.xyz', // 地址栏的完整地址，方便测试用
      domain: '', // 当前的域名
  ...
  computed: {
    ...
    pcBackgroundImgUrl() {
      if (this.$store.state.tenant.tenantInfo &&
        this.$store.state.tenant.tenantInfo.pcBackImage &&
        this.$store.state.tenant.tenantInfo.pcBackImage.fileUrl
      ) {
        return `url(${this.$store.state.tenant.tenantInfo.pcBackImage.fileUrl})`
      } else {
    ...
    handleLogin() {
      this.$refs.loginForm.validate(async(valid) => {
        if (valid) {
          ...
          login({ ...this.loginForm, tenantId: this.tempTenantId, userType: '1' }, getDeviceType())
            .then((ret) => {
