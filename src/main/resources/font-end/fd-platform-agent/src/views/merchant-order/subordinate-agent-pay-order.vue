  ...
  <div class="container">
    ...
    <el-card>
      <el-form :inline="true" :model="queryFormData" class="demo-form-inline">
        ...
        <el-form-item label="至" prop="merchant_order_money" class="zhi">
          <el-input
            v-model="queryFormData.merchant_order_money2"
            placeholder="订单金额"
            size="small"
            style="width: 180px;"
          >
      ...
      >
        ...
        <el-table-column label="订单号" align="center" width="180">
          <template slot-scope="{ row }">
            <div>
              <div><el-tag>系统</el-tag> {{ row.system_order_num }}</div>
              <div class="mt-5"><el-tag>商户</el-tag> {{ row.merchant_order_num }}</div>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="金额" align="center">
          <template slot-scope="{ row }">
            <div>
              <div><el-tag>订单</el-tag> {{ row.merchant_order_money }}</div>
              <!-- <div class="mt-5" >
                <el-tag>实付</el-tag> {{ row.pay_order_money }}
              </div> -->
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="交易者信息" align="center">
          <template slot-scope="{ row }">
            <div>
              ...
              <div class="mt-5"><el-tag>码商</el-tag> {{ row.trans_product_no }}</div>
        ...
        <el-table-column label="结果说明" align="center" prop="submit_info">
          ...
          <template slot-scope="{ row }">
            <div class="opt-box">
              ...
              <!-- <div type="primary" class="btn mt-5" @click="look_trans_product_info(row)">
                <i class="el-icon-delete" />族级
              </div>

              <div type="primary" class="btn mt-5" @click="showFenpeiDialog(row)">
                <i class="el-icon-delete" />分配
              </div> -->

              <!-- <div type="primary" class="btn mt-5" @click="showChuliDialog(row, 0)">
                <i class="el-icon-delete" />处理订单
              </div> -->

              <div type="primary" class="btn mt-5" @click="showCommonDialog(row, 0)">
                <i class="el-icon-delete" />取消订单
              </div>

              <div type="primary" class="btn mt-5" @click="showCommonDialog(row, 1)">
                <i class="el-icon-delete" />确认订单
              </div>

              <div type="primary" class="btn mt-5" @click="showCommonDialog(row, 2)">
                <i class="el-icon-delete" />驳回订单
              </div>
    ...
    <el-dialog :visible.sync="commonDialogVisible" :title="commonDialogTitle" width="600px">
      ...
      >
        ...
        <el-form-item label="商户订单号" v-if="[0,1,2].includes(commonDialogType)" prop="system_order_num">
          <el-input
            disabled
            v-model="commonFormData.system_order_num"
            placeholder="请输入系统订单号"
        ...
        <el-form-item label="收款信息" v-if="[0,1,2].includes(commonDialogType)" prop="system_order_num">
          <el-input
            disabled
            v-model="commonFormData.system_order_num"
            placeholder="请输入系统订单号"
        ...
        <el-form-item v-if="commonDialogType == 3"  required label="审核说明" >
          <el-select filterable  filterable  v-model="commonFormData.checkReason" placeholder="请选择原因" size="small">
            <el-option
              v-for="i in 5"
              :key="i"
              :label="`说明`+i"
        ...
        <el-form-item v-if="commonDialogType == 3 && commonFormData.checkReason == 6"  required label="其他说明" prop="system_order_num">
          <el-input
        ...
        <el-form-item v-if="commonDialogType == 2"  required label="驳回原因" >
          <el-select filterable  filterable  v-model="commonFormData.rejectReson" placeholder="请选择原因" size="small">
            <el-option
              v-for="i in 5"
              :key="i"
              :label="`原因`+i"
        ...
        <el-form-item v-if="commonDialogType == 2 && commonFormData.rejectReson == 6"  required label="其他原因" prop="system_order_num">
          <el-input
            v-model="commonFormData.otherRejectReson"
            placeholder="请输入驳回原因"
            size="small"
          />
        </el-form-item>

        <el-form-item v-if="[1,2].includes(commonDialogType)" required label="凭证" prop="system_order_num">
          <el-upload
          ...
          >
            ...
            <div v-else  style="width: 50px;height:50px; border: 1px solid #dfe4ed; display: flex; justify-content: center; align-items: center;  " >
        ...
        <el-form-item label="交易密码"  v-if="[3,4].includes(commonDialogType)"  prop="password" >
          <el-input
    ...
    <el-dialog :visible.sync="showChuliDialogVisual" title="处理订单" width="600px">
      ...
      >
        ...
        <el-form-item label="商户订单号" prop="system_order_num">
          <el-input
            disabled
            v-model="commonFormData.system_order_num"
            placeholder="请输入系统订单号"
        ...
        <el-form-item label="收款信息" prop="system_order_num">
          <el-input
            disabled
            v-model="commonFormData.system_order_num"
            placeholder="请输入系统订单号"
        ...
        <el-form-item label="驳回原因" prop="system_order_num">
          <el-input
            v-model="commonFormData.system_order_num"
            placeholder="请输入驳回原因"
            size="small"
        ...
        <el-form-item label="凭证上传" prop="system_order_num">
          <el-upload
            class="avatar-uploader"
            action="#"
            multiple
    ...
    <el-dialog :visible.sync="fenpeiDialogVisual" title="分配订单" width="600px">
      ...
      >
        <el-form-item label="系统订单号" prop="system_order_num">
          <el-input
            disabled
            v-model="fenpeiFormData.system_order_num"
            placeholder="请输入系统订单号"
            size="small"
          />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    look_shoukuan_info(row) {
      this.$alert(`查看${row.system_order_num} 的收款码`, {
        confirmButtonText: '确定',
        callback: (action) => {
          // this.$message({
          //   type: 'info',
