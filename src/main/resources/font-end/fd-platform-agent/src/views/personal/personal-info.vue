    ...
    <el-card>
      <div class="request">
        ...
        <p class="req-line">
          ...
          <span class="req-value">{{ formData.deviceType ?formData.deviceType :'---'}}</span>
        </p>
        <p class="req-line">
          <span class="req-label">登录IP：</span>
          <span class="req-value">{{ formData.ip ?formData.ip :'---'}}</span>
        </p>
        <p class="req-line">
          <span class="req-label">最后登录时间：</span>
          <span class="req-value">{{ formData.lastLoginTime ?formData.lastLoginTime :'---'}}</span>
        </p>
        <p class="req-line">
        ...
        <p class="req-line">
          ...
          <span class="req-value">{{ formData.tenantProfitRate ?formData.tenantProfitRate :0}}%</span>
        </p>
...
<script>
import { getTenantAgentInfo } from '@/api/user'
export default {
  name: 'PersonalInfo',
  data() {
    return {
  ...
  methods: {
    async getTenantAgentInfo() {
      let res = await getTenantAgentInfo()
      this.formData = res.data
    }
  },
  mounted() {
    this.getTenantAgentInfo()
  }
}
</script>
