  ...
  <div class="container">
    ...
    <el-card class="mt-10">
      ...
      <my-table
        class="mt-10"
        ref="tableRef"
        :loadData="platformDailyReportGetReportPage"
        :tableData="tableData"
        style="width: 100%"
        border
      >
        ...
        <el-table-column label="成功实付金额(元)" align="center" prop="successfulPayAmount" width="140"/>
...
<script>
import {
  platformDailyReportGetReportPage,
  platformDailyReportSummary,
...
export default {
  ...
  methods: {
    async platformDailyReportGetReportPage(page) {
      let obj = {
      ...
      let res = await platformDailyReportGetReportPage(obj, page)
      this.tableData = res.data.list
      console.log(res)
      return res
    },
    async platformDailyReportSummary() {
      let obj = {
        tenantId: this.tenantId
      }
      Object.keys(this.queryFormData).forEach((key) => {
      ...
      let res = await platformDailyReportSummary(obj)
    ...
    resetQueryForm() {
      ...
      this.$refs.tableRef.getTableData(1)
      this.platformDailyReportSummary()
