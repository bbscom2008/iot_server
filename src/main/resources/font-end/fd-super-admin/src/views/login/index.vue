...
import defaultBg from '@/assets/img/default-bg.png'
  ...
  methods: {
    ...
    handleLogin() {
      this.$refs.loginForm.validate(async (valid) => {
        if (valid) {
          ...
          login({ ...this.loginForm, tenantId: this.tempTenantId, userType: '0' }, getDeviceType())
            .then((ret) => {
    ...
    async getTenantDomainByDomainName() {
      ...
      if (res.data) {
        ...
        this.$store.state.tenant.currTenantId = this.tempTenantId
        this.$store.state.tenant.selectTenantId = this.tempTenantId
        this.$message.error('未找到域名信息，请联系管理员')
      }
    },
