  ...
  <div class="container">
    <el-card>
      <el-form :inline="true" :model="queryFormData" class="demo-form-inline">
        <el-form-item label="群组id" prop="">
          <el-input v-model="queryFormData.chatId" placeholder="群组id" size="small" />
        </el-form-item>

        <el-form-item label="群组类型" prop="">
          <el-select
            filterable
            v-model="queryFormData.chatType"
            placeholder="请选择群组类型"
            size="small"
          >
            <el-option label="全部" value=""></el-option>
            <el-option label="商户群" :value="0"></el-option>
            <el-option label="码商群" :value="1"></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="群名称" prop="">
          <el-input v-model="queryFormData.chatTitle" placeholder="群名称" size="small" />
        </el-form-item>

        <el-form-item label="消息主题" prop="">
          <el-input v-model="queryFormData.theme" placeholder="消息主题" size="small" />
        </el-form-item>
    ...
    <el-card class="mt-10">
      ...
      >
        ...
        <el-table-column label="群组ID" align="center" prop="chatId" />
        <el-table-column label="群组名称" align="center" prop="chatTitle" />

        <el-table-column label="群组类型" align="center" prop="chatType">
          <template slot-scope="{ row }">
            <el-tag v-if="row.chatType == 0">商户群</el-tag>
            <el-tag v-if="row.botUserType == 1">码商群</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="绑定用户数量" align="center" prop="bingUserNumber">
          <template slot-scope="{ row }">
            <el-button type="text">{{ row.bingUserNumber }}</el-button>
          </template>
        </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item
          v-if="formData.botUserType === 3 || formData.botUserType === 4"
          label="群组id"
          prop="botChatId"
        >
          <el-input v-model="formData.botChatId" placeholder="请输入群组ID" size="small" />
        </el-form-item>
...
<script>
import { getBotBindGroupRelationPage } from '@/api/bot-group-list.js'
import { botUserDelete, botUserUpdate, botUserAdd } from '@/api/bot-user-list.js'
...
export default {
  ...
  methods: {
    ...
    async loadTableData(page) {
      ...
      let res = await getBotBindGroupRelationPage(page, params)
