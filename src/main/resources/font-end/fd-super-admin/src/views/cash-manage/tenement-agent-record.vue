...
<script>
import { getTenantAgentAccountChangePage,tenantAgentAccountChangeExport } from '@/api/cash-manage'
...
export default {
  ...
  watch: {
    selectTenantId(newVal, oldVal) {
      ...
      if (this.$route.name == 'tenementAgentRecord') {
  ...
  methods: {
    ...
    async onDialogBtnOk() {
      ...
      let res = await tenantAgentAccountChangeExport(rest)
      this.$message.success('导出成功')
      this.onDialogBtnCancel()
      console.log(res, 'res')
      FileSaver.saveAs(res, '平台代理账变记录.xlsx')
    },
    exportReport() {
    ...
    async getTenantAgentAccountChangePage(page) {
      // let obj = { ...this.queryFormData, tenantId: '0' }
      ...
      let res = await getTenantAgentAccountChangePage(params, { ...page })
