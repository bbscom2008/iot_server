<template>
  <div class="container">
    ...
    <el-card class="mt-10">
      <my-table
        class="mt-10"
        :loadData="getTenantAgentWithdrawalPage"
        :tableData="tableData"
        ref="tableRef"
        style="width: 100%"
        border
    ...
    <el-dialog
      @close="showShenHeDialogBtnCancel()"
      :visible.sync="showShenHeDialogVisual"
      title="提现审核"
      width="600px"
    >
      <el-form
      ...
      >
        ...
        <el-form-item label="提现金额(元)" prop="">
          <el-input
            type="number"
...
<script>
import { getTenantAgentWithdrawalPage, tenantAgentWithdrawalAudit } from '@/api/merchant-cash'
...
export default {
  ...
  methods: {
    async getTenantAgentWithdrawalPage(page) {
      ...
      let res = await getTenantAgentWithdrawalPage(rest, { ...page })
    ...
    showShenHeDialogBtnOk(e) {
      this.$refs.formData.validate(async (valid) => {
        if (valid) {
          ...
          let res = await tenantAgentWithdrawalAudit({
            withdrawalApplyId,
            auditDesc,
            tradePassword,
