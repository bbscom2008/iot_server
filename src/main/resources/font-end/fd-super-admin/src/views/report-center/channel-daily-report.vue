...
import {
  ...
  dailyChannelReportSummary,
  dailyChannelReportExport,
  dailyChannelReportGenerate
...
export default {
  ...
  watch: {
    selectTenantId(newVal, oldVal) {
      ...
      if (this.$route.name == 'ChannelDailyReport') {
  ...
  methods: {
    ...
    async getChannelReportPage(page) {
      let obj = {
      ...
      let res = await getChannelReportPage(obj, page)
    ...
    async exportReport() {
      ...
      let res = await dailyChannelReportExport(obj)
      console.log(res, 'res')
      FileSaver.saveAs(res, '通道日报表.xlsx')
    },
    showCreateDialog() {
    ...
    async onDialogBtnOk() {
      ...
      let res = await dailyChannelReportGenerate({
