  ...
  <div class="container" :class="isShowUi ? 'isShowUi' : 'noUi'">
    ...
    <el-card class="mt-10">
      ...
      <my-table
        class="mt-10"
        ref="tableRef"
        :loadData="getProductTypeReportPage"
        :tableData="tableData"
        style="width: 100%"
        border
      >
        ...
        <el-table-column label="成功率" align="center" sortable prop="successfulRate" width="95">
          <template slot-scope="{ row }">
            {{ Number(row.successfulRate) }}
          </template>
        </el-table-column>
...
import {
  getProductTypeReportPage,
  dailyProductTypeReportSummary,
  dailyProductTypeReportExport,
  dailyProductTypeReportGenerate
...
export default {
  ...
  watch: {
    selectTenantId(newVal, oldVal) {
      ...
      if (this.$route.name == 'ProductTypeDaily') {
  ...
  methods: {
    ...
    async getProductTypeReportPage(page) {
      let obj = {
      ...
      let res = await getProductTypeReportPage(obj, page)
    ...
    async exportReport() {
      ...
      let res = await dailyProductTypeReportExport(obj)
      console.log(res, 'res')
      FileSaver.saveAs(res, '产品类型日报表.xlsx')
    },
    showCreateDialog() {
    ...
    async onDialogBtnOk() {
      ...
      let res = await dailyProductTypeReportGenerate({
