...
<script>
import { providerDailyReport, providerDailyReportSummary } from '@/api/today-report'
import TradeTypeSwitch from '@/components/TradeTypeSwitch'
import { dailyServiceReportExportToday } from '@/api/today-report'
...
export default {
  ...
  methods: {
    ...
    async exportReport() {
      ...
      let res = await dailyServiceReportExportToday(obj)
      FileSaver.saveAs(res, '码商今日报表.xlsx')
    },
    async providerDailyReport(page) {
      let obj = {
        tenantId: this.selectTenantId,
        userId: this.$store.state.user.userId,
        tradeType: this.tradeType
      }
      let res = await providerDailyReport({ ...obj, ...page })
