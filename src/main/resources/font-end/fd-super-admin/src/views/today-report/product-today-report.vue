...
<script>
import { dailyProductReport, dailyProductReportSummary } from '@/api/today-report'
import TradeTypeSwitch from '@/components/TradeTypeSwitch'
import { dailyProductReportExportToday } from '@/api/today-report'
...
export default {
  ...
  methods: {
    ...
    async exportReport() {
      ...
      let res = await dailyProductReportExportToday(obj)
      FileSaver.saveAs(res, '产品今日报表.xlsx')
    },

    async dailyProductReport(page) {
      let obj = {
        tenantId: this.selectTenantId,
        userId: this.$store.state.user.userId,
        tradeType: this.tradeType
      }
      let res = await dailyProductReport({ ...obj, ...page })
