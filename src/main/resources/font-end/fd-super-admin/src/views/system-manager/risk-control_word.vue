<template>
  <div class="container">
    <el-card>
      ...
      >
        <el-form-item label="敏感词" prop="word">
          <el-input v-model="queryFormData.word" placeholder="请输入敏感词" size="small" />
        </el-form-item>
    ...
    >
      ...
      >
        <el-form-item label="敏感词" prop="word" :rules="wordRules">
          <el-input v-model="formData.word" placeholder="请输入敏感词" size="small" />
        </el-form-item>
...
export default {
  ...
  methods: {
    async sensitiveWordPage(page) {
      ...
      let res = await sensitiveWordPage(params, page)
    ...
    async onSwitchChange(e, row) {
      try {
        // let res = await sensitiveWordEnable(row.sensitiveWordId, { enabled: e ? true : false })
        let res = await sensitiveWordEnable(row.sensitiveWordId, { enabled: e  })
        console.log(res)
      } catch (e) {
        console.log(e)
      }
    ...
    async showConfirmDelete(row) {
      try {
        let ret = await this.$confirm(`确定删除敏感词【 ${row.word} 】 吗？`)
        let res = await sensitiveWordDelete(row.sensitiveWordId)
        //todo 调用接口删除
        console.log('删除成功')
        this.$refs.tableRef.getTableData()
      } catch (e) {
      ...
      this.$refs.formData.validate(async (valid) => {
        if (valid) {
          if (this.isEdit) {
            ...
            //新增
            let res = await sensitiveWordAdd({ ...this.formData, tenantId: this.$store.state.tenant.selectTenantId })
            console.log(res, '添加')
          }
