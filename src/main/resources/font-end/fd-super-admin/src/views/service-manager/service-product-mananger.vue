  ...
  <div class="container" :class="isShowUi ? 'isShowUi' : 'noUi'">
    ...
    <el-card class="mt-10 my-card">
      ...
      >
        ...
        <el-table-column label="交易类型" align="center" v-if="selectedItems.includes('交易类型')">
          <template slot-scope="{ row }">
            <el-tag :type="row.tradeType == 0 ? 'primary' : 'danger'">{{
              row.tradeType == 0 ? '代收' : '代付'
            }}</el-tag>
          </template>
        </el-table-column>

        <!-- <el-table-column label="接单下线" align="center">
          <template slot-scope="{ row }">
            <el-tag
              :type="row.isReceiveOffline ? 'success' : 'danger'"
            >{{ formData.isReceiveOffline ? '开启' : '关闭' }}</el-tag>
        ...
        <el-table-column label="收款码" width="110" v-if="selectedItems.includes('收款信息数量')">
          <template slot-scope="{ row }">
            <div class="ta-l ml-10">
              <div>
                总数<span>
        ...
        <el-table-column
          ...
          align="center"
          min-width="120"
          v-if="selectedItems.includes('浮动金额')"
        >
          <template slot-scope="{ row }">
            <div>
          ...
          <el-table-column label="排除值" align="center">
            <template slot-scope="{ row }">
              {{row.floatExclude}}
            </template>
          </el-table-column>
          <el-table-column label="是否允许重置" align="center">
            <template slot-scope="{ row }">
              {{row.floatAllowDuplicate?'允许':'不允许'}}
            </template>
          </el-table-column> -->
        ...
        <el-table-column
          label="超时次数限制"
          align="center"
          v-if="selectedItems.includes('超时限制')"
        >
          <template slot-scope="{ row }">
            <div v-if="row.orderConsecutiveTimeoutLimit">
              ...
              <div style="text-align: center">
                <span style="color: #c00; font-weight: 600">{{
                  row.orderConsecutiveTimeoutLimit
                }}</span>
                次
              </div>
        ...
        <el-table-column label="系统费率" align="center" v-if="selectedItems.includes('系统费率')">
          <template slot-scope="{ row }">
            <span v-if="row.systemRate != null && row.systemRate != undefined">
              {{ row.systemRate }}%
            </span>
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="{ row }">
            <el-tag v-if="row.isOneTimeAccept == 1" type="danger">接单下线</el-tag>
        ...
        <el-table-column align="center" label="操作" width="120" fixtt="right">
          <template slot-scope="{ row }">
            <div class="opt">
    ...
    >
      ...
      >
        ...
        <!-- <el-form-item label="接单下线" prop="isReceiveOffline">
          <el-select
            filterable
            clearable
            v-model="formData.isReceiveOffline"
            placeholder="请设置接单下线"
            size="small"
          >
            <el-option label="默认开启" :value="1" ></el-option>
            <el-option label="默认关闭" :value="0" ></el-option>
          </el-select>
        </el-form-item> -->
        ...
        <div class="tips">
          该产品下的任意一个收款码连续超时接单的次数，超过了设置的次数，那这个收款信息就不能用了。设置为
          0 将不限制
        </div>
        ...
        <!-- <template v-if="formData.tradeType == 0">
          <el-form-item label="是否开启浮动" prop="isFloatEnable">
            <el-switch disabled v-model="formData.isFloatEnable" />
          </el-form-item>
          ...
          <el-form-item
            ...
            :rules="flostSettingRules.floatMax"
          >
            <el-input
              v-number-input
              type="number"
          ...
          <el-form-item
            ...
            prop="floatMin"
            v-if="formData.isFloatEnable"
            :rules="flostSettingRules.floatMin"
          >
            <el-input
              v-number-input
              type="number"
    ...
    >
      ...
      >
        ...
        <template v-if="floatReadOnly">
          ...
          <el-form-item label="浮动最大值：" prop="" v-if="formData.isFloatEnable">
            <div>{{ Number(formData.floatMax) }}</div>
          </el-form-item>

          <el-form-item label="浮动最小值：" prop="" v-if="formData.isFloatEnable">
            <div>{{ Number(formData.floatMin) }}</div>
          </el-form-item>
          <el-form-item label="浮动排除值：" prop="" v-if="formData.isFloatEnable">
            <div>{{ formData.floatExclude }}</div>
          </el-form-item>
          <el-form-item label="是否允许浮动重复：" prop="" v-if="formData.isFloatEnable">
    ...
    >
      ...
      <!-- <div slot="footer" style="text-align: center">
        <el-button @click="closeProviderDialog()" size="small">
          {{ '关闭' }}
        </el-button>
      </div> -->
    </el-dialog>
    ...
    >
      ...
      <!-- <div slot="footer" style="text-align: center">
        <el-button @click="closePayAccountDialog()" size="small"> 关闭 </el-button>
      </div> -->
    </el-dialog>
    <!-- 产品配置码商 -->
    <el-dialog
...
import { getProductTypeList } from '@/api/tenant-manager.js'

import { getProductListByTenantIdContainsNotActive } from '@/api/service-list-setting.js'
...
export default {
  ...
  methods: {
    ...
    async productTypeChange(e) {
      ...
      // this.formData.cashTemplateId = res.data.cashTemplateId
        ...
        if (valid) {
          if (this.isEdit) {
            ...
            await updateProduct({
              ...
              // isReceiveOffline: this.formData.isReceiveOffline,
    ...
    flushPage() {
      ...
      // 同步刷新 产品缓存
      getProductListByTenantIdContainsNotActive(this.$store.state.tenant.selectTenantId).then(
        (res) => {
    ...
    getTenantListInfo() {
      getAllTemplate(this.$store.state.tenant.selectTenantId).then((res) => {
        console.log(res)
      ...
      getProductTypeList(this.$store.state.tenant.selectTenantId).then((res) => {
        console.log(res)
