  ...
  <div class="container">
    ...
    <el-card class="mt-10">
      ...
      >
        ...
        <el-table-column label="收款信息" align="center" prop="userName" min-width="200">
          <template slot-scope="{ row }">
            <div style="display: flex; gap: 5px; justify-content: space-between">
              <el-tag type="primary">{{row.level}}级码商</el-tag>
              <div
        ...
        <el-table-column label="授权状态" align="center" min-winth="110">
          <template slot-scope="{ row }">
        ...
        <el-table-column label="今日成功订单信息" align="center" min-width="130">
          <template slot-scope="{ row }">
        ...
        <el-table-column align="center" label="操作" width="110" fixed="right">
          <template slot-scope="{ row }">
            <div class="opt">
              ...
              <div type="success" class="btn" @click="showMobileAuth(row)">
              ...
              <div type="success" class="btn" @click="showMonitorAddr(row)">
...
export default {
  ...
  watch: {
    '$store.state.tenant.selectTenantId'(newV, oldV) {
      console.log('-----------selectTenantId-----------------', newV)
      // 根据租户ID 获得所有的产品信息
      this.getOptionLists()
      this.queryTableData()
    }
  ...
  methods: {
    ...
    handleQrConfigError(error, file, fileList) {
      console.log('----handleQrConfigError----', error, file, fileList)
    ...
    async handleQrConfigSuccess(response, file, fileList) {
      console.log('----handleQrConfigSuccess----', response, file, fileList)
      const fileName = file.name
      // 之前失败的再次上传，成功了
      const qrConfig = this.formData.payeeAccountConfig.qrConfig
    ...
    showAddDialog() {
      ...
      this.formData = {
        ...
        // productId: ''
        // isActive: 1,
        // inputNameType: 0, // 0 手动输入  1 由图片自动生成
      }
    ...
    async onDialogBtnOk() {
      ...
      try {
        ...
        if (this.isEdit) {
          try {
            const editParams = this.procPayeeAccountInfoEditParams()
            editParams.payeeAccountName = this.formData.payeeAccountName
            const acceptParams = this.buildAcceptOrderParams(this.acceptOrderForm, [
              this.formData.payeeAccountId
            ])

            Promise.all([
              updatePayeeAccountInfo(editParams),
    ...
    async updatePatchCreatedPayee(createdIds,acceptOrderForm, isEnd=false){
      try {
        ...
        const msg =
          ...
          error?.message ||
          (typeof error === 'string' ? error : '')
        this.$message.error(msg || '接单配置保存失败，请稍后在列表中通过"接单配置"按钮重试')
      } finally {
        if(isEnd){
          this.onDialogBtnCancel()
          this.flushPage()
...
::v-deep .add-form .file-config .el-upload {
  display: block;
}

::v-deep .look-info-form {
  .el-form-item {
    margin-bottom: 0px;
  }
  .el-dialog__body {
