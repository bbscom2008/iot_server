  ...
  <div class="container">
    ...
    <el-card class="mt-10">
      ...
      <el-button type="primary" plain icon="el-icon-edit-outline" @click="showPatchEditDialog(true)"
        >全部修改</el-button
      >
      <el-button type="danger" icon="el-icon-edit-outline" @click="unBindAll()"
        >全部解绑【超管用】</el-button
      >
      ...
      >
        ...
        <el-table-column label="码商账号" align="center" prop="providerUserName" />
        <el-table-column label="码商名称" align="center" prop="providerName"> </el-table-column>
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="{ row }">
            ...
            <el-button type="text" plain size="small" @click="showProductRateHistoryDialog(row)">
    ...
    >
      ...
      >
        ...
        <el-form-item label="码商名称" :prop="isEdit ? '' : 'providerIdList'">
          <el-input
            v-if="isEdit"
            disabled
          ...
          <el-select
            v-else
            filterable
            v-model="formData.providerIdList"
            :multiple="true"
            placeholder="请选择"
            @change="productChange"
          >
            <el-option v-if="serviceList.length!=0" label="全部" value=""></el-option>
            <el-option
              v-for="item in serviceList"
              :label="item.userNickName"
              :value="item.providerId"
...
import {
  getProviderProductRelation,
  batchUnbindProduct,
  unbindProduct,
  updateProduct,
  createProductProviderRelation,
  batchUpdateProduct,
  updateProductProviderRelationByProductId,
...
export default {
  ...
  computed: {
    productListFilter() {
      //   return this.allProductList.filter((ele) => ele.tradeType == this.formData.tradeType)
    }
  },
  ...
  methods: {
    async unBindAll(){
      // 显示一个确认框
      try{
        let ret = await this.$confirm("确认全部解绑吗")

        this.tableData.forEach(row=>{
            unbindProduct(row.id)
        })
    ...
    onPatchBtnOk() {
      this.$refs.formPatch.validate(async (valid) => {
        ...
        if (!this.isModAll) {
          ...
          // 全部修改
          await updateProductProviderRelationByProductId({
            productId: this.currItem.productId,
            isModifiedProviderRate: this.formPatch.isModifiedProviderRate,
            providerRateAdjustType: this.formPatch.providerRateAdjustType,
            providerRate: this.formPatch.providerRate,
            isProviderPerTransactionFee: +this.formPatch.isProviderPerTransactionFee,
    ...
    showAddDialog() {
      ...
      this.formData = {
        weight: 1,
        providerNickName: this.currItem.productName,
        providerIdList: [],
        productName: '',
    ...
    onDialogBtnOk() {
      this.$refs.formData.validate(async (valid) => {
        ...
        if (this.isEdit) {
          ...
          let providerIdsFlag = this.formData.providerIdList
          const isAllprovider = this.formData.providerIdList.indexOf('') != -1
          if (isAllprovider) {
            providerIdsFlag = []
          }

          await createProductProviderRelation({
            tenantId:this.$store.state.tenant.selectTenantId,
            productId: this.currItem.productId,
            providerIdList: providerIdsFlag,
            providerRate: this.formData.providerRate,
            providerPerTransactionFee: this.formData.providerPerTransactionFee,
            weight: this.formData.weight,
            isActive: this.formData.isActive
    ...
    async loadTableData(page) {
      ...
      let res = await getProviderProductRelation(params, page)
