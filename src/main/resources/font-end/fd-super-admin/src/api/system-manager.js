 ...
 * @returns 
 */
export function systemConfigIsExists(configKey) {
  store.commit('app/setLoadingEnable', false)
  return request({
    url:`/master/system/config/systemConfigIsExists/${configKey}`,
    method: 'get'
  })
}
/**
...
export function systemConfigUpdate(data) {
  return request({
    url:'/master/system/config/update',
    method: 'post',
    data
  })
}
...
/**
 * 系统管理---根据configId获取系统配置
 * @returns 
 */
export function systemConfig(configId) {
  return request({
    url: `/master/system/config/${configId}`,
    method: 'get'
  })
}
...
export function getLoginLogPage(data,params) {
  ...
  /**
 * 根据用户ID获取用户信息---查看用户是否被锁定
 * @returns 
 */
export function systemUserIsLock(userId) {
    return request({
      url: `/master/system/user/${userId}`,
      method: 'get'
    })
  }
//////////////////////////////////
