...
export function payeeAccountInfoNameIsExist(payeeAccountName, providerId) {
  ...
  return request({
    url:
      tempBase +
      `/master/payeeAccount/existsPayeeAccountName/${payeeAccountName}?providerId=${providerId}`,
    method: 'get'
  })
}
...
export function getProductInfoListByProviderId(providerId) {
  return request({
    url:
      tempBase +
      `/master/ProviderProductRelation/getPayeeAccountProductListByProviderId/${providerId}`,
    method: 'get'
  })
}
...
export function getPayeeAccountInfoCount(payeeAccountId) {
  ...
  return request({
    url:
      tempBase +
      `/statistics/dailyPayeeAccountInfoReport/getSummaryTodayAndYesterdayByPayeeAccountId?payeeAccountId=${payeeAccountId}`,
    method: 'get'
  })
}
...
export function isOneTimeAcceptIsActive(payeeAccountId, isOneTimeAccept) {
  ...
  return request({
    url:
      tempBase +
      `/master/payeeAccount/isOneTimeAccept/${payeeAccountId}?isOneTimeAccept=${isOneTimeAccept}`,
    method: 'get'
  })
}

// 根据payeeAccountId批量删除收款账户信息
export function payeeAccountBatchDelete(payeeAccountIds) {
  return request({
    url: tempBase + `/master/payeeAccount/batchDelete`,
    method: 'post',
    data: payeeAccountIds
  })
}
// 超管或平台端-导出收款账户信息
export function exportPayeeAccountList(data) {
  return request({
    url: `/master/payeeAccount/exportPayeeAccountList`,
    method: 'post',
    data,
    responseType: 'blob'
  })
 ...
 * 通过租户Id获取收款信息（下拉框）
 */
export function getPayeeAccountInfoListByTenantId(tenantId) {
  return request({
    url: tempBase + `/master/payeeAccount/getPayeeAccountOptionListByTenantId/${tenantId}`,
    method: 'get'
  })
}
...
export function getProxyIp() {
  store.commit('app/setLoadingEnable', false)
  return request({
    url: tempBase + `/master/wechat/proxy/getProxyIp`,
    method: 'post',
    data: {}
  })
}
 ...
 * 获得微信登录 二维码
 */
export function getWeixinLoginQrcode(payeeAccountId, data) {
  return request({
    url: tempBase + `/master/wechat/login/getQR/${payeeAccountId}`,
    method: 'post',
    data
  })
}
 ...
 * 根据收款信息ID获取微信用户信息 
 */
export function getWechatUserInfo(payeeAccountId) {
  return request({
    url: tempBase + `/master/wechat/getWechatUserInfo/${payeeAccountId}`,
    method: 'get'
  })
}

// 通道配置商户的费率记录
export function getChannelMerchantRateHistoryList(merchantChannelRelationId, {pageNum,pageSize}) {
  return request({
    url: tempBase + `/master/MerchantChannelRelation/getChannelMerchantRecordPage?pageNum=${pageNum}&pageSize=${pageSize}`,
    method: 'post',
    data: {
      merchantChannelRelationId
    }
  })
}
