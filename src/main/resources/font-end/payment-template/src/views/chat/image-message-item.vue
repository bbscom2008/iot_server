...
export default {
  ...
  methods: {
    ...
    reload() {
      this.isError = false;
      if (this.item.isLocal) { // 本地图片，需要上传到服务器
    ...
    async parseImageMessage() {
      ...
      try {
        ...
        addOrderOperationLog(
          this.$store.getters.orderId,'收到图片消息： '+this.item.fileId
        );

        let res = await getFileById(this.item.fileId);
        this.item.imgSrc = res.data.fileUrl;
        addOrderOperationLog(
          this.$store.getters.orderId,'获得图片地址： '+this.item.imgSrc
        );
        ...
        this.isError = true;
        addOrderOperationLog(
          this.$store.getters.orderId,'获得图片地址失败： '+ err
        );
      } finally {
        this.isLoading = false;
      }
    ...
    async uploadImage(messageItem) {
      ...
      try {
        ...
        let res = await uploadFileToServer(imageItem.file, (progress) => {
