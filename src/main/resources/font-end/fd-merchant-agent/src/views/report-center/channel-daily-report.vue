...
<script>
import { dailyChannelReportSummary, dailyChannelReportPage } from '@/api/report-center'
export default {
  ...
  methods: {
    async getDataList() {
      let ret = await dailyChannelReportPage()
      console.log(ret, 'retretret')
    },
    async dailyChannelReportSummary() {
      let ret = await dailyChannelReportSummary()
      console.log(ret,'retretret')
    ...
    mockData() {
      ...
      let columnList = [
        ...
        { title: '商户名称', type: 'string', key: 'userNickName', isSearch: true, id: 1002 },
      ...
      new Array(10).fill(0).forEach((_, index) => {
        ...
        columnList.forEach((column) => {
          ...
          //
          if (column.title == '通道名称') {
            ele[column.key] = '通道' + this.tableData.length
          }
