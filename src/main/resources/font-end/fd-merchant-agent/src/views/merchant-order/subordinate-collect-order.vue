<template>
  <div class="container">
        ...
        <el-card class="item-card" v-for="(ele, i) in cardItems" :key="i">
          ...
          <div class="item">
            <div class="txt" v-for="item in ele" :key="item.title">
              ...
              <span class="money">{{ item.value }}  {{ item.type=='number'?'单':item.type=='money'?'元':'%' }}  </span>
      ...
      >
        ...
        <el-table-column label="订单号" align="center" width="180">
          <template slot-scope="{ row }">
            <div>
              <div >
                <el-tag >系统</el-tag> {{ row.system_order_num }} 
              </div>
              <div  class="mt-5">
                <el-tag>商户</el-tag> {{ row.merchant_order_num }}
              </div>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="金额" align="center">
          <template slot-scope="{ row }">
            <div>
              <div>
                <el-tag>订单</el-tag> {{ row.merchant_order_money }}
              </div>
              <div class="mt-5" >
        ...
        <el-table-column label="交易者信息" align="center">
          <template slot-scope="{ row }">
            <div>
              <div>
                <el-tag>商户</el-tag> {{ row.trans_merchant_no }}
              </div>
              <div class="mt-5">
                <el-tag>码商</el-tag> {{ row.trans_product_no }}
              </div>
    ...
    <el-dialog :visible.sync="trans_product_visual" title="交易者信息" width="600px">
      <my-table class="mt-10"   style="width: 100%" border :tableData="tableData"
        :loadData="mockData"
        :total="50">
        <el-table-column label="序号" align="center" type="index" width="60" />
    ...
    <el-dialog :visible.sync="submit_info_dialog_visible" title="查看提交信息" width="600px">
      ...
      >
        ...
        <el-form-item label="" >
          <el-button type="primary" @click="submit_info_formData.isShow = true" size="small"> 查看 </el-button>
          <div v-if="submit_info_formData.isShow" >查看 {{ submit_info_formData.system_order_num }} 的提交信息 

            <el-button type="text" @click="e=>{clipboard(submit_info_formData.system_order_num+'的提交信息', e)}" >复制</el-button>
          </div>
        </el-form-item>
      </el-form>
      <div slot="footer">
