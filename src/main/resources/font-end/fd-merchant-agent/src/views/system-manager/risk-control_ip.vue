  ...
  <div class="container">
    <el-card>
      <el-form :inline="true" :model="queryFormData" class="demo-form-inline">
        <el-form-item label="IP地址" prop="ip_add">
          <el-input v-model="queryFormData.ip_add" placeholder="请输入IP地址" size="small" />
        </el-form-item>

        <el-form-item label="是否启用" prop="is_enable">
          <el-select v-model="queryFormData.is_enable" placeholder="请选择是否启用" size="small">
      ...
      >
        ...
        <el-table-column label="IP" align="center"  >
          <template slot-scope="{ row }">
            <span>{{ `${Math.floor(Math.random()*254)+1}.${Math.floor(Math.random()*254)+1}.${Math.floor(Math.random()*254)+1}.${Math.floor(Math.random()*254)+1}` }}</span>
          </template>
        </el-table-column>
        <el-table-column label="地址" align="center" prop="ip_add" />
    ...
    >
      ...
      >
        <el-form-item label="IP地址" prop="ip_add">
          <el-input v-model="formData.ip_add" placeholder="请输入IP地址" size="small" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    queryTableData() {
      ...
      if (this.queryFormData.ip_add != '') {
        this.tableData = this.tableData.filter((item) => {
          return item.ip_add == this.queryFormData.ip_add
        })
      }
    ...
    mockData() {
      ...
      let columnList = [
        ...
        { id: 1011, title: 'IP', type: 'number', key: 'ip', isSearch: true },
        { id: 1011, title: '地址', type: 'string', key: 'ip_add', isSearch: true },
        { id: 1011, title: '是否启用', type: 'boolean', key: 'is_enable', isSearch: true },
        { title: '创建时间', type: 'date', key: 'create_time', isSearch: true, id: 1008 },
        { title: '备注', type: 'string', key: 'remark', isSearch: false, id: 1009 },
        { title: '操作', type: 'operate', key: 'opt', isSearch: false, id: 1010 }
      ]
      this.tableData = []
